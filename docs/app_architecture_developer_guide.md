...
> **Note (2026-03-11):** Code examples in this guide reference projects, meetings, and chat as illustrative domains. The application has since pivoted to DappaAi AI (AlayaCare caregiver shift-filling). The underlying layered architecture patterns described here remain unchanged and apply equally to the new AlayaCare integration. External API proxy routes (e.g., `/api/v1/visits`) use a simplified variant of the pattern — see `docs/ARCHITECTURE.md` for details.

_A deep-dive companion to the Mayfly architecture, written for developers who are new to layered application design, backend patterns, and production-grade TypeScript. This guide also serves as a north star for architectural principles across all projects — the patterns described here are not Mayfly-specific, and the [Future Developments](#future-developments-scaling-the-architecture) section provides guidance on when to adopt more advanced patterns as applications grow._
...
- **Code that represents the target architecture** — marked with a 📌 icon. These are patterns we plan to adopt. When you see 📌, it means "this is the blueprint, but it's not in the codebase yet."

Why does this matter? Because as a junior developer, you need to know whether you're looking at something you can go open in your editor right now, or something that represents where we're headed. Both are valuable — the existing code shows you how things work today, and the 📌 items show you the direction of travel so you build new features the right way from the start.
...
```ts
  return deps.uow.run(ctx, async (tx) => {
    ...
    await deps.outbox.enqueue(tx, {
      ...
      payload: { version: 1, projectId: record.id, ownerId: ctx.principalId, name: input.name },
      traceId: ctx.traceId,
    });
...
> 📌 _Implementation status:_ In `mayfly-starter`, `handleCreateProject` currently only depends on `{ uow }` plus service functions. The snippet above represents the canonical target architecture. When you add repositories, policies, idempotency, and the outbox, follow this pattern.
...
> _Implementation status:_ The starter today wires `{ uow, readOnly, projectPolicy, meetingPolicy, chatPolicy }` as a cached singleton. The UoW and ReadOnly classes import the `db` connection internally — they don't receive it as a constructor argument. Migrate to lazy getters as the dependency count grows beyond 10-15 objects.
...
> 📌 _Implementation status:_ The outbox table and worker are part of the target architecture but haven't been wired into `mayfly-starter` yet.
...
> 📌 _Implementation status:_ Mayfly doesn't run heavy batch jobs yet. These patterns are documented here for when they're needed.
