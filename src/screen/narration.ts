 ...
 * style criteria. Spoken once per session, doubles as the session
 * greeting (no separate opener fires before this).
 *
 * The universe count is woven into the intro rather than narrated as
 * a separate step — Tavus's stopped_speaking event was firing before
 * the intro buffer drained, letting the universe line cut in
 * mid-sentence. One echo, one beat between thoughts via punctuation.
 */
export function describeQuestionnaireIntro(universeCount: number): string {
  return `Hi, I'm Pep. Let me tell you about the OC Funds Management stock filtering approach. OC is a benchmark-unaware Australian equities manager — they take a stock-specific, bottom-up approach and assess each company on its own merits, not its index weight. The methodology I'll walk you through filters out a few categories of company by design: those that are too small, unprofitable, too speculative, or insufficiently liquid. We're starting from the full ASX universe — about ${universeCount.toLocaleString()} listed companies in this snapshot — and I'll apply each criterion one at a time so you can see how the universe narrows. You can run through the filters in order by clicking the Next filter button, or just click on the filter you want to run, or just tell me what filter you want me to run — but don't forget to click on the unmute button first.`;
}
