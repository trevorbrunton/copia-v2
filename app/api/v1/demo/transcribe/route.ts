 ...
 *
 * Receives raw PCM Int16 LE audio, wraps it in WAV, and transcribes
 * via ElevenLabs Speech-to-Text API.
 *
  ...
  try {
    ...
    if (!audioFile || !(audioFile instanceof Blob)) {
      return Response.json(
        { error: { code: "INVALID_INPUT", message: "audio file is required" } },
        { status: 400 },
      );
    }
    ...
    // Guard against oversized uploads (~1 min at 48kHz mono 16-bit = ~5.7MB)
    const MAX_AUDIO_BYTES = 10_000_000; // 10MB
    if (pcmBuffer.byteLength > MAX_AUDIO_BYTES) {
      return Response.json(
        { error: { code: "INVALID_INPUT", message: `audio exceeds maximum size of ${MAX_AUDIO_BYTES} bytes` } },
        { status: 400 },
      );
    }
    ...
    logger.info(
      ...
      "demo:transcribe request",
    );

    // Wrap PCM in WAV header for ElevenLabs
    ...
    if (!res.ok) {
      ...
      logger.error(
        ...
        "demo:transcribe ElevenLabs STT failed",
      );
      throw new ExternalServiceError(
    ...
    logger.info(
      { traceId, textLength: text.length },
      "demo:transcribe response",
    );
