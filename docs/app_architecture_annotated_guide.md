...
> **Note (2026-03-11):** Code examples reference projects and meetings as illustrative domains. The application has pivoted to DappaAi AI (AlayaCare integration). The architectural patterns remain the same. See `docs/ARCHITECTURE.md` for current system overview.

_A plain-language reference for the Mayfly platform, with annotated code samples and pragmatic guardrails._
...
📌 _Implementation status:_ The outbox table and worker are part of the standard but have not been wired into `mayfly-starter` yet. Use these exact names when you add them so the shared tooling (workers, tests, CI rules) slots in cleanly.
...
📌 _Implementation status:_ Until the outbox + worker land in this codebase, keep async side effects behind feature flags or fire them manually after the write succeeds. Migrating to the canonical flow should be the first task before escalating those features to Tier A.
